...
import { getTestimonials, Testimonial } from "@/lib/dataService";

const Testimonials = () => {
  const { data: displayTestimonials = [], isLoading } = useQuery<Testimonial[]>({
                ...
                <CardContent className="p-0 flex-1 flex flex-col">
                  ...
                  <div className="flex items-center mt-auto">
                    <div className="w-12 h-12 bg-primary rounded-full mr-4 flex items-center justify-center">
                      <span className="text-white font-bold">{testimonial.name.split(' ').map(n => n[0]).join('').toUpperCase()}</span>
                    </div>
                    <div>
                      <h4 className="font-merriweather font-bold text-primary">{testimonial.name}</h4>
                      <p className="text-sm text-foreground">{testimonial.role} at {testimonial.company}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
